...
/**
  * Spark流功能的主要入口点。它提供了用于创建来自[[org.apache.spark.streaming.dstream.DStream]]的各种输入源。
  * 它可以通过提供Spark主URL和appName来创建，也可以通过org.apache.spark.SparkConf配置（请参阅核心Spark文档），
  * 或从现有的org.apache.spark.SparkContext。可以使用访问关联的SparkContext`context.sparkContext`.
  * 在创建和转换数据流之后，可以分别使用启动和停止流计算`context.start()`和`context.stop()`。
  * `context.awaitTermination()`允许当前线程等待通过`stop()`或异常终止上下文。
  *
...
class StreamingContext private[streaming](
    ...
    /**
      * 创建一个新的数据流，其中每个RDD都是通过在数据流的RDD上应用函数生成的。
      *
...
/**
  * StreamingContext对象包含许多与StreamingContext类相关的实用函数。
  *
