...
class PairDStreamFunctions[K, V](self: DStream[(K, V)])
    ...
    /**
      * 通过对每个RDD应用“groupByKey”返回新的数据流。哈希分区用于生成具有Spark默认分区数的RDD。
      *
    ...
    /**
      * 通过对每个RDD应用“groupByKey”返回新的数据流。哈希分区用于生成具有“numPartitions”分区的RDD。
      *
      * Return a new DStream by applying `groupByKey` to each RDD. Hash partitioning is used to
    ...
    /**
      * 通过将函数应用于“this”流的每个键值元素，同时为每个唯一键维护一些状态数据，
      * 返回一个[[MapWithStateDStream]]。此转换的映射函数和其他规范（例如分区、超时、初始状态数据等）
      * 可以使用“StateSpec”类指定。状态数据可以作为映射函数中“state”类型的参数在中访问。
      *
      * :: Experimental ::
    ...
    /**
      * 返回一个新的“状态”数据流，其中每个键的状态通过在键的先前状态和每个键的新值上应用给定函数来更新。
      * 哈希分区用于生成具有Spark默认分区数的RDD。
      *
