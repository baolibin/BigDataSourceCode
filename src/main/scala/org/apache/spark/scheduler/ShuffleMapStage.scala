...
/**
  * ShuffleMapStages是执行DAG中的中间阶段，它为一次shuffle操作生成数据。
  * 它们正好出现在每个shuffle操作之前，并且在此之前可能包含多个流水线操作（例如map和filter）。
  * 执行时，它们保存map输出文件，这些文件稍后可以由reduce任务获取。
  * “shuffleDep”字段描述每个阶段所属的shuffle，并且像“outputLocs”和“numAvailableOutputs”这样的变量跟踪准备好的map输出的数量。
  *
  * ShuffleMapStages也可以作为具有DAGScheduler.submitMapStage文件.
  * 对于这些阶段，提交它们的ActiveJobs将在“mapStageJobs”中跟踪。
  * 请注意，可能有多个activejob试图计算同一个shuffle map stage。
  *
