...
/**
 * 1、JDK1.7和JDK1.8实现版本的不同：
 * 在JDK1.7版本中ConcurrentHashMap采用了：数组+分段锁（ReentrantLock+Segment+HashEntry）的方式实现。
 * 在JDK1.8版本中ConcurrentHashMap采用了：synchronized+CAS+HashEntry+红黑树。
 * <p>
 * 2、CAS简介：
 * cas是一种基于锁的操作，而且是乐观锁。在java中锁分为乐观锁和悲观锁。
 * 悲观锁是将资源锁住，等一个之前获得锁的线程释放锁之后，下一个线程才可以访问。乐观锁采取了一种宽泛的态度，通过某种方式不加锁来处理资源。
 * CAS 操作包含三个操作数 —— 内存位置（V）、预期原值（A）和新值(B)。如果内存地址里面的值和A的值是一样的，那么就将内存里面的值更新成B。
 * <p>
 * 3、JDK1.8相比JDK1.7版本中ConcurrentHashMap中实现：
...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
	 ...
	 * 实际的浮点值通常不被使用——使用{@coden-（n>>>2）}这样的表达式作为相关的调整大小阈值更简单。
	 *
