...
/**
  * 这是为了处理需要键和使用流畅接口（现在是join和coGroup）的操作。对于每个操作，我们需要一个实现“finish”的子类来使用提供的键创建实际操作。
  *
  ...
  *
  * 这样，我们就有了一个中心点，在这个中心点上，所有的密钥提供都发生了，如果支持的密钥类型发生了变化，就不需要更改特定的操作。
  *
  ...
  *
  * 我们使用类型参数“O”来指定结果操作的类型。对于join，它将是“JoinDataSet[L，R]”，对于coGroup，它将是“CoGroupDataSet[L，R]”。这样用户就可以为完成的操作获得正确的类型。
  *
