...
/**
  * 一种仅附加的缓冲区，类似于ArrayBuffer，但是对于小的缓冲区内存效率更高。
  * ArrayBuffer总是分配一个对象数组来存储数据，默认情况下有16个条目，因此它有大约80-100字节的开销。
  * 相反，CompactBuffer最多可以在主对象的字段中保留两个元素，并且只有在有更多的条目时才分配数组[AnyRef]。
  * 这使得它对于groupBy这样的操作更有效，在groupBy中，我们希望某些键的元素非常少。
  *
