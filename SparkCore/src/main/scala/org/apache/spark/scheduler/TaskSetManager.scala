...
/**
  * 在TaskSchedulerImpl的单个任务集中安排任务。
  * 此类跟踪每个任务，在任务失败时重试任务（次数有限），并通过延迟调度处理此任务集的位置感知调度。
  * 它的主要接口是resourceOffer，它询问TaskSet是否要在一个节点上运行任务；statusUpdate，
  * 它告诉TaskSet它的一个任务更改了状态（例如，finished）。
  *
  * TaskSchedulerImpl调度tasks在单独的TaskSet中,TaskSetManager保持追踪每个task.
  *
...
private[spark] class TaskSetManager(
    ...
    /**
      * 取消给定节点的挂起任务的队列，并返回其索引和位置级别。仅搜索与给定位置约束匹配的任务。
      *
