...
/**
  * BitTorrent实现类似[[org.apache.spark.broadcast.Broadcast]].
  *
  ...
  *
  * 驱动程序将序列化对象分成小块，并将这些小块存储在驱动程序的BlockManager中。
  *
  * The driver divides the serialized object into small chunks and
  * stores those chunks in the BlockManager of the driver.
  *
  * 在每个执行器上，执行器首先尝试从其BlockManager获取对象。
  * 如果不存在，则使用远程获取从驱动程序和/或其他执行器（如果可用）获取小块。
  * 一旦获得块，它就将块放入自己的BlockManager中，准备好让其他执行者从中获取。
  *
  ...
  *
  * 这可以防止驱动程序成为发送广播数据的多个副本（每个执行器一个）的瓶颈。
  *
...
private[spark] class TorrentBroadcast[T: ClassTag](obj: T, id: Long)
    ...
    private def setConf(conf: SparkConf) {
        compressionCodec = if (conf.getBoolean("org.apache.spark.broadcast.compress", true)) {
            Some(CompressionCodec.createCodec(conf))
        } else {
            None
        }
        // Note: use getSizeAsKb (not bytes) to maintain compatibility if no units are provided
        blockSize = conf.getSizeAsKb("org.apache.spark.broadcast.blockSize", "4m").toInt * 1024
        checksumEnabled = conf.getBoolean("org.apache.spark.broadcast.checksum", true)
    }
