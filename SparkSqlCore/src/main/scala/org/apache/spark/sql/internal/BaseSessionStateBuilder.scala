...
/**
  * 用于协调新[[SessionState]]的构造的生成器类。
  *
  * Builder class that coordinates construction of a new [[SessionState]].
  *
  * 生成器显式定义会话状态所需的所有组件，并在调用“build”时创建会话状态。组件只能初始化一次。
  * 这对于大多数组件来说都不是问题，因为它们只在“build”函数中使用。
  * 但是，有些组件（`conf`、`catalog`、`functionRegistry`、`experimentalMethods`和`sqlParser`）作为其他组件的依赖项，因此共享。
  * 这些组件被定义为惰性VAL，以确保只创建一次组件。
  *
  ...
  *
  * 开发人员可以通过提供组件的自定义版本，或者通过使用analyzer、optimizer和planner提供的挂钩来修改生成器。
  * 组件之间存在一些依赖关系（每个依赖关系都有文档记录），开发人员在进行修改时应该尊重这些依赖关系，以防止出现初始化问题。
  *
  ...
  *
  * 父级[[SessionState]]可用于初始化新的[[SessionState]]。
  * 新会话状态将克隆父会话状态的“conf”、“functionRegistry”、“experimentalMethods”和“catalog”字段。
  * 请注意，状态是在调用“build”时克隆的，而不是在调用之前克隆的。
  *
