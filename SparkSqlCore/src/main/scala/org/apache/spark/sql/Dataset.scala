...
/**
  * Dataset是特定领域的对象的强类型集合，可以使用函数或关系操作进行并行转换。
  * 每个数据集还有一个称为“DataFrame”的非类型化视图，它是[[Row]]的数据集。
  *
  * A Dataset is a strongly typed collection of domain-specific objects that can be transformed
  * in parallel using functional or relational operations. Each Dataset also has an untyped view
  * called a `DataFrame`, which is a Dataset of [[Row]].
  *
  * 数据集上可用的操作分为转换和操作。转换是产生新数据集的转换，动作是触发计算并返回结果的转换。
  * 示例转换包括map、filter、select和aggregate（`groupBy`）。
  * 示例操作计数、显示或将数据写入文件系统。
  *
  ...
  *
  * 为了有效地支持特定领域的对象，需要一个[[Encoder]]。
  * 编码器将特定于域的类型“T”映射到Spark的内部类型系统。
  * 例如，给定一个类“Person”，它有两个字段“name”（string）和“age”（int），编码器用来告诉Spark在运行时生成代码，将“Person”对象序列化为二进制结构。
  * 这种二进制结构通常具有较低的内存占用，并针对数据处理的效率进行了优化（例如，以列格式）。
  * 要理解数据的内部二进制表示，请使用“schema”函数。
  *
