...
/**
  * [[ConnectedStreams]]表示两个（可能）不同数据类型的连接流。
  * 连接流对于一个流上的操作直接影响另一个流上的操作的情况很有用，通常通过流之间的共享状态。
  *
...
class ConnectedStreams[IN1, IN2](javaStream: JavaCStream[IN1, IN2]) {
    ...
    def flatMap[R: TypeInformation](
        ...
        val flatMapper = new CoFlatMapFunction[IN1, IN2, R] {
            def flatMap1(value: IN1, out: Collector[R]) = {
                cleanFun1(value) foreach out.collect
            }

            def flatMap2(value: IN2, out: Collector[R]) = {
                cleanFun2(value) foreach out.collect
            }
        }
